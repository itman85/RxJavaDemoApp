...
public class ServerPollingAfterDataProcessingExampleActivity extends BaseActivity {
    ...
    @BindView(R.id.tv_server_polling_count)
    TextView tvServerPollingCount;
    @BindView(R.id.tv_local_data_processed)
    TextView tvLocalDataProcessingCount;
    ...
    private Subscription processData_manualRecursion_with_repeatWhen_utilCondition() {
                ...
                .map((string) -> {
                    ...
                    Observable
                            ...
                            .flatMap((aLong) -> {
                                Log.v(TAG, "timer() - Calling emitData()...");
                                //return emitData();
                                return Observable.just(pollingAttempt--);
                            })
                            .repeatWhen((observable) -> {
                                Log.v(TAG, "repeatWhen()");
                                return observable.delay(10, TimeUnit.MILLISECONDS);
                            })
                            .takeUntil(integer -> integer==2)//repeat until pollingAttempt == 2, so when pollingAttempt < 2 will not receive emit
                            .doOnCompleted(() -> {
                                Log.v(TAG, "doOnCompleted()");
                            })
                            .doOnNext(integer -> {
